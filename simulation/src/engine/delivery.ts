...
/**
 * What `deliver` needs to POST one event. Deliberately minimal — just the HTTP
 * client and an optional timeout — so the worker pool can call `deliver`
 * without providing the scheduling hooks `runEvents` uses. The scheduler's
 * wider `RunDeps` extends this.
 */
